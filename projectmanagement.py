...
def CreateDB():
    ...
    # create a table
    c.execute(""" CREAT TABLE IF NOT EXISTS strategy(
            strategy_name text,
            port_number integer,
...
def AddData(name):
    if c.fetchone() not None: # checking if DB is not empty
        port=c.execute(""" SELECT MAX(port_number)) FROM strategy """)
        c.execute(""" INSERT INTO strategy VALUES (name,port+1)""")
        c.commit()
    else:
        port=5001
        c.execute(""" INSERT INTO strategy VALUES (name,port)""")
        c.commit()
#-----------------------------------------------------------------------------------
...
def OpenResearchBook(name):
    ...
    if len(name)>0: # CHECK IF NAME IS ENTERED
        ...
        if check is None:
            ...
            port_num = c.fetchone()
            jbook="lean research " + name + " --port "+ port_num # we'll fetch,check and store it once SQLLite part is figured.
